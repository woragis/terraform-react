locals {
  ...
  certificate_name      = "${local.normalized_project}-${local.environment}-cert"  # No variable override for this, keeping as is
  cert_validation       = tolist(aws_acm_certificate.cert.domain_validation_options)[0]

  # Tags
  common_tags           = {
    Project     = var.project_name
    Environment = local.environment  # Use normalized environment
    ManagedBy   = "Terraform"
  }

  # CloudFront Settings
  ssl_minimum_protocol_version = var.ssl_minimum_protocol_version  # Use variable instead of hardcoding
